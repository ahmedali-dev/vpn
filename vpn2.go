...
func (v *VPNClient) startSOCKSServer() error {
	...
	// Start HTTP/HTTPS proxy if port is specified
	if v.config.HTTPPort > 0 {
		go v.startHTTPProxy()
	}
...
func (v *VPNClient) printBanner() {
	...
	if v.config.HTTPPort > 0 {
		fmt.Printf("HTTP/HTTPS Proxy: 127.0.0.1:%d\n", v.config.HTTPPort)
	}
	...
	if v.config.AutoRoute {
		...
		fmt.Println("   ⚠️  You MUST configure applications to use proxy below")
	}
	...
	if v.config.PACPort > 0 {
		...
		fmt.Println("\n💡 PAC Configuration:")
		if v.config.HTTPPort > 0 {
			fmt.Println("   • HTTP/HTTPS traffic → HTTP Proxy")
			fmt.Println("   • Other traffic → SOCKS5 Proxy")
		} else {
			fmt.Println("   • All traffic → SOCKS5 Proxy")
		}
		fmt.Println("   • Local network → Direct connection")
		fmt.Println("   • Works automatically when VPN is running")
		
	} else {
		fmt.Println("\n╔═══════════════════════════════════════════════════════════╗")
		fmt.Println("║  ⚠️  MANUAL CONFIGURATION REQUIRED!                      ║")
		fmt.Println("╚═══════════════════════════════════════════════════════════╝")
		...
		if v.config.HTTPPort > 0 {
			fmt.Println("\n🔧 Option 1: HTTP/HTTPS Proxy (Recommended for browsers)")
			fmt.Println("   Firefox / Chrome / Edge:")
			fmt.Println("   • HTTP Proxy: 127.0.0.1")
			fmt.Printf("   • Port: %d\n", v.config.HTTPPort)
			fmt.Println("   • Use for: HTTP and HTTPS")
			fmt.Println("   • Also use for FTP: Yes")
		}
		
		fmt.Println("\n🔧 Option 2: SOCKS5 Proxy (Universal)")
		fmt.Println("   Firefox:")
		fmt.Println("   1. Settings → Network Settings → Manual proxy")
		fmt.Println("   2. SOCKS Host: 127.0.0.1")
		fmt.Printf("   3. Port: %d\n", v.config.SOCKSPort)
		fmt.Println("   4. Select: SOCKS v5")
		fmt.Println("   5. ✅ CHECK: 'Proxy DNS when using SOCKS v5'")
	}
	...
	if v.config.HTTPPort > 0 {
		fmt.Printf("\n   With HTTP proxy:\n")
		fmt.Printf("   curl -x http://127.0.0.1:%d https://ipinfo.io/ip\n", v.config.HTTPPort)
	}
...
func (v *VPNClient) testConnection() {
	...
	// Test HTTP proxy if enabled
	if v.config.HTTPPort > 0 {
		httpConn, err := net.DialTimeout("tcp", fmt.Sprintf("127.0.0.1:%d", v.config.HTTPPort), 5*time.Second)
		if err != nil {
			fmt.Printf("[!] Cannot connect to HTTP proxy: %v\n", err)
		} else {
			httpConn.Close()
			fmt.Println("[✓] HTTP/HTTPS proxy is listening on port", v.config.HTTPPort)
		}
	}
...
func (v *VPNClient) startHTTPProxy() {
	if v.config.HTTPPort <= 0 {
		return
	}

	addr := fmt.Sprintf("127.0.0.1:%d", v.config.HTTPPort)
	listener, err := net.Listen("tcp", addr)
	if err != nil {
	...
	fmt.Printf("[+] HTTP/HTTPS proxy started on %s\n", addr)
	...
	for v.running.Load() {
		...
		go v.handleHTTPProxy(conn)
	}
}

// handleHTTPProxy handles HTTP/HTTPS proxy connections
func (v *VPNClient) handleHTTPProxy(clientConn net.Conn) {
	defer clientConn.Close()
	...
	// Read first line to determine if CONNECT (HTTPS) or regular HTTP
	buf := make([]byte, 4096)
	...
	if method == "CONNECT" {
		// HTTPS tunnel
		v.handleHTTPSConnect(clientConn, target)
	} else {
		// Regular HTTP proxy
		v.handleHTTPRequest(clientConn, request, target)
	}
}

// handleHTTPSConnect handles HTTPS CONNECT method
func (v *VPNClient) handleHTTPSConnect(clientConn net.Conn, target string) {
	// Parse host:port
	host := target
	...
	for retry := 0; retry < 3; retry++ {
		...
		remoteConn, err = v.sshClient.Dial("tcp", host)
		if err == nil {
			break
		}
...
// handleHTTPRequest handles regular HTTP proxy requests
func (v *VPNClient) handleHTTPRequest(clientConn net.Conn, request string, target string) {
	...
	for retry := 0; retry < 3; retry++ {
		...
		remoteConn, err = v.sshClient.Dial("tcp", addr)
		if err == nil {
			break
		}
	...
	// Rebuild request without proxy-specific headers
	lines := strings.Split(request, "\r\n")
	var newRequest strings.Builder
	
	// Rewrite request line with path only
	requestParts := strings.Fields(lines[0])
	if len(requestParts) >= 3 {
		newRequest.WriteString(fmt.Sprintf("%s %s %s\r\n", requestParts[0], target, requestParts[2]))
	}
	
	// Copy other headers except Proxy-Connection
	for i := 1; i < len(lines); i++ {
		line := lines[i]
		if !strings.HasPrefix(strings.ToLower(line), "proxy-") {
			newRequest.WriteString(line)
			newRequest.WriteString("\r\n")
		}
	}

	// Send request to remote
	remoteConn.Write([]byte(newRequest.String()))
...
func parseFlags() Config {
	...
	httpPort := flag.Int("http-port", 0, "HTTP/HTTPS proxy port (e.g., 8888)")
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  Basic with SOCKS5 only:\n")
		fmt.Fprintf(os.Stderr, "    %s -H 51.159.125.86 -P 8443 -u user -p pass -S web.whatsapp.com\n\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  With HTTP/HTTPS proxy (better for browsers):\n")
		fmt.Fprintf(os.Stderr, "    %s -H 51.159.125.86 -P 8443 -u user -p pass -S web.whatsapp.com -http-port 8888\n\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  With automatic browser configuration (RECOMMENDED):\n")
		fmt.Fprintf(os.Stderr, "    %s -H 51.159.125.86 -P 8443 -u user -p pass -S web.whatsapp.com -http-port 8888 -pac-port 8080\n\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  Full setup with auto-route:\n")
		fmt.Fprintf(os.Stderr, "    %s -H 51.159.125.86 -P 8443 -u user -p pass -S web.whatsapp.com -http-port 8888 -pac-port 8080 -auto-route\n\n", os.Args[0])
		
		if runtime.GOOS == "windows" {
			fmt.Fprintf(os.Stderr, "Windows Users:\n")
			fmt.Fprintf(os.Stderr, "  • Use -http-port for better browser compatibility\n")
			fmt.Fprintf(os.Stderr, "  • Use -pac-port for automatic configuration (easiest!)\n")
		...
		fmt.Fprintf(os.Stderr, "  SOCKS5 (-socks-port):\n")
		fmt.Fprintf(os.Stderr, "    • Universal proxy for all protocols\n")
		fmt.Fprintf(os.Stderr, "    • Required, always enabled (default: 1080)\n\n")
		
		fmt.Fprintf(os.Stderr, "  HTTP/HTTPS (-http-port):\n")
		fmt.Fprintf(os.Stderr, "    • Optimized for web browsing\n")
		fmt.Fprintf(os.Stderr, "    • Better compatibility with browsers\n")
		fmt.Fprintf(os.Stderr, "    • Recommended for most users\n\n")
		
		fmt.Fprintf(os.Stderr, "  PAC Server (-pac-port):\n")
		fmt.Fprintf(os.Stderr, "    • Automatic proxy configuration\n")
		fmt.Fprintf(os.Stderr, "    • Configure once in system/browser settings\n")
		fmt.Fprintf(os.Stderr, "    • Works automatically when VPN is running\n")
		fmt.Fprintf(os.Stderr, "    • Easiest method for all users!\n\n")
...
func (v *VPNClient) generatePACFile() string {
	if v.pacServer.httpPort > 0 {
		// Use HTTP proxy for HTTP/HTTPS, SOCKS5 for everything else
		return fmt.Sprintf(`function FindProxyForURL(url, host) {
    // VPN Proxy Auto-Configuration
    // HTTP/HTTPS via HTTP proxy, other protocols via SOCKS5
    ...
    // HTTP and HTTPS traffic goes through HTTP proxy (faster for web browsing)
    if (url.substring(0, 7) == "http://" || 
        url.substring(0, 8) == "https://") {
        return "PROXY 127.0.0.1:%d; SOCKS5 127.0.0.1:%d; DIRECT";
    }
    
    // Everything else (FTP, etc.) goes through SOCKS5
    return "SOCKS5 127.0.0.1:%d; SOCKS 127.0.0.1:%d; DIRECT";
}`, v.pacServer.httpPort, v.pacServer.socksPort, v.pacServer.socksPort, v.pacServer.socksPort)
	}
